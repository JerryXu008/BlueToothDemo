...
//订阅characteristics
/*
要想这个回调走，特征必须具有通知权限 CBCharacteristicPropertyNotify
 */
...
-(BOOL)sendData:(NSTimer *)t {
    ...
    //执行回应Central通知数据
   BOOL result = [peripheralManager updateValue:[[dft stringFromDate:[NSDate date]] dataUsingEncoding:NSUTF8StringEncoding] forCharacteristic:(CBMutableCharacteristic *)characteristic onSubscribedCentrals:nil];
    return result;
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral didReceiveReadRequest:(CBATTRequest *)request{
    ...
    if (request.characteristic.properties & CBCharacteristicPropertyRead) {
        ...
        //把读取的结果返回
        data  = [@"这是读取的结果" dataUsingEncoding:NSUTF8StringEncoding];
        [request setValue:data];
        
        if([request.characteristic.UUID.UUIDString isEqualToString:@"FFF2"]){
            character = request.characteristic;
        }
